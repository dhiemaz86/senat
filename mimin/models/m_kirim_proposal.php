...
class kirim_proposal{
	...
	public function bacadwn($id){
		 

		 $kuweri = "select * from kirim_proposal kp join orma o on kp.id_orma = o.id_orma where kp.id_proposal='".$id."'";
 		 //$orma = mysql_fetch_array(mysql_query($quweri));	

 		 return $this->db->getRows($kuweri);		
	}

	public function readAllKirim_proposal(){
		$query = "Select * from kirim_proposal kp join orma o on kp.id_orma=o.id_orma join status s on kp.status=s.id_status ";
		return $this->db->getRows($query);	
	}

	public function readKirim_proposal($id){
		$query = "Select * from kirim_proposal kp join orma o on kp.id_orma=o.id_orma where id_proposal='".$id."'";
		return $this->db->getRows($query);		
	}
...
public function updater_proposal($id, $data){
	 $tgl = date('Y-m-d');
		...
		$query="UPDATE kirim_proposal SET id_orma='$ids', no_surat='$nomor', judul='$judul', proposal='$proposal', deskripsi='$deskripsi', status='$status', tgl_input='$tgl' WHERE id_proposal=$id"; 
