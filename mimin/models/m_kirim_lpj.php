...
class kirim_lpj{
	...
	public function readAllKirim_lpj(){
		$query = "Select * from kirim_lpj kp join orma o on kp.id_orma=o.id_orma join status s on kp.status=s.id_status ";
		return $this->db->getRows($query);	
	}

	public function readKirim_lpj($id){
		$query = "Select * from kirim_lpj kp join orma o on kp.id_orma=o.id_orma where id_lpj='".$id."'";
		return $this->db->getRows($query);		
	}
	...
	public function updater_lpj($id, $data){
	 $tgl = date('Y-m-d');
		...
		$query="UPDATE kirim_lpj SET id_orma='$ids', no_surat='$nomor', judul='$judul', lpj='$lpj', deskripsi='$deskripsi', status='$status', tgl_input='$tgl' WHERE id_lpj=$id"; 
		//$query = "update kirim_lpj set judul='$judul', nomor='$nomor', lpj='$lpj, deskripsi='$deskripsi'";
